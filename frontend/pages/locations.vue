...
import MobileDesktop from "@/components/MobileDesktop/MobileDesktop.vue";
...
const switchTreeView = ref(false);
...
<template>
  <BaseContainer>
      ...
      <div>
        <TooltipProvider :delay-duration="0">
          <ButtonGroup>
            ...
            <Tooltip>
              <TooltipTrigger>
                <Button size="icon" variant="outline" @click="switchTreeView = !switchTreeView">
                  <MdiExpandAllOutline />
                </Button>
              </TooltipTrigger>
              <TooltipContent>
                <p>{{ $t("locations.switch_tree_view") }}</p>
              </TooltipContent>
            </Tooltip>
            <Tooltip>
              <TooltipTrigger>
    ...
    <!-- 手机桌面视图 -->
    <div v-else>
      <MobileDesktop :tree="tree" :current-location="'root'" />
    </div>
  </BaseContainer>
</template>
